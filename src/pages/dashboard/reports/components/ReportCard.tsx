...
export function ReportCard({ report, wrapperExtraClasses }: ReportCardProps) {
  ...
  return (
    <article className={`bg-placeholder flex h-full flex-col rounded-sm p-5 ${wrapperExtraClasses}`}>
      ...
      <section className="flex gap-2">
        ...
        <Link
          to={`/video-games/${report.videoGame?.slug}`}
          className="font-bebas hover:text-green relative z-20 flex items-center pt-0.5 text-xl leading-none text-white uppercase not-hover:no-underline"
        >
          {report.videoGame?.titulo}
        </Link>
      </section>
      <section className="flex gap-2">
