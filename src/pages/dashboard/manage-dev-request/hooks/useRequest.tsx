...
export function useRequest(): returnedRequest & { refetch: () => void } {
    ...
    const fetchRequest = () => {
        ...
        axios.get(`/api/users/developers/solicitudes?limit=${itemsPerPage}&offset=${page}&orderBy=fecha&order=DESC`)
            .then((resp) => {
                const respRequest = resp.data.data.filter((req: SolicitudDesarrollador) =>
                    req.estado === 'PENDING'
                )
                setData(respRequest)
...
export function useSearch(): returnedSearch {
    ...
    useEffect(() => {
        axios.get(`/api/users/developers/solicitudes?search=${query}`)
            .then((resp) => {
...
export function useGetRequest(id: string) {
    ...
    useEffect(() => {
        axios.get(`/api/users/developers/${id}/solicitud`)
            .then((resp) => {
                setData(resp.data);
            }).catch((e) => {
                console.error(e)
