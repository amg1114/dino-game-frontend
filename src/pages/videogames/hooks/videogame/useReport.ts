...
export function useReport(slug: string) {
    ...
    const handleReportSubmit = (reportType: number, id: number) => {
        ...
        axios.post(`/api/reports/${id}`, { typeReportId: reportType })
            .then(() => {
                showAlert({
                    type: 'success',
            ...
            .catch((e) => {
                showAlert({
                    ...
                    message: e.response?.data?.message || 'Ocurrió un error al enviar el reporte. Por favor, inténtalo de nuevo más tarde.',
                    duration: 3000,
                });
            });
    }
